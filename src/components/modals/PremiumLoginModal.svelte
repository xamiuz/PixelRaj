    ...
    <form on:submit|preventDefault={handleSubmit} class="login-form">
      <div class="premium-input-group">
        ...
        <div class="input-wrapper">
          ...
          <input id="email" type="text" bind:value={email} required placeholder={$t('login.email_placeholder')} autocomplete="username" />
        </div>
      </div>
      
      <div class="premium-input-group">
        <label for="password">{$t('login.pass_label')}</label>
        <div class="input-wrapper">
          <KeyRound size={18} class="input-icon" />
          <input id="password" type={showPassword ? "text" : "password"} value={loginPassword} on:input={(e) => loginPassword = e.target.value} required placeholder={$t('login.pass_placeholder')} minlength="6" />
        ...
        {#if !isSignUp}
          <div style="text-align: right; margin-top: 6px;">
            <button type="button" class="link-btn" style="font-size: 12px; color: var(--figma-text-muted);" on:click={handleForgotPassword}>{$t('login.forgot_pass')}</button>
          </div>
        {/if}
      </div>
      ...
      <div style="text-align: center; margin-top: 16px; font-size: 13px; color: #a1a1aa;">
        {#if isSignUp}
          {$t('login.have_account')} <button type="button" class="link-btn" on:click={() => isSignUp = false}>{$t('login.login_here')}</button>
        {:else}
          {$t('login.no_account')} <button type="button" class="link-btn" on:click={() => isSignUp = true}>{$t('login.signup_here')}</button>
        {/if}
      </div>
    </form>
  </div>
