...
<div class="modal-overlay premium-login-overlay" class:visible={isMounted}>
  <div class="modal-card">
    ...
    <form on:submit|preventDefault={handleSubmit} class="login-form">
      ...
      <div class="premium-input-group">
        ...
        <div class="input-wrapper">
          ...
          <input
            ...
            value={loginPassword}
            on:input={(e) => (loginPassword = e.target.value)}
            required
        ...
        {#if !isSignUp}
          <div style="text-align: right; margin-top: 6px;">
            <button
              type="button"
              class="link-btn"
              style="font-size: 12px; color: var(--figma-text-muted);"
              on:click={handleForgotPassword}>{$t("login.forgot_pass")}</button
            >
          </div>
        {/if}
      </div>
      ...
      <div
        style="text-align: center; margin-top: 16px; font-size: 13px; color: #a1a1aa;"
      >
        {#if isSignUp}
          ...
          <button
            type="button"
            class="link-btn"
            on:click={() => (isSignUp = false)}>{$t("login.login_here")}</button
          >
        {:else}
          {$t("login.no_account")}
          <button
            type="button"
            class="link-btn"
            on:click={() => (isSignUp = true)}>{$t("login.signup_here")}</button
          >
        {/if}
      </div>
    </form>
